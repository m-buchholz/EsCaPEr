...
public class prefabsSymbols : MonoBehaviour
{
    public List<string> UtensilList = new List<string>()
    {
    ...
    public GameObject GeneratePrefab(string name)
    {
        ...
        spriteObject.GetComponent<FadeInSymbols>().FadeOutSymbol(1, utensil);
        spriteObject.GetComponent<FadeInSymbols>().FadeOutSymbol(2, utensil);
    ...
    private void setSettingToSpriteObject(GameObject spriteObject, string name)
    {
        ...
        spriteObject.AddComponent<FadeInSymbols>();
        spriteObject.AddComponent<DragAndDrop>();
        spriteObject.AddComponent<CollisionControllerForAssignmentSymbols>();
        spriteObject.name = name;
        spriteObject.GetComponent<SpriteRenderer>().sprite = Resources.Load<Sprite>("Images/Symbols/" + name);
